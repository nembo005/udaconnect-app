...
logger = logging.getLogger("udaconnect-location-saver")

def save_sample_location(location):
    # Verify dictionary
    try:
        location_dict : Dict = json.loads(location)
    except:
        logger.warning(
            f"Non-dict payload detected : {location}")
        return
    ...
    if validation_result:
        logger.warning(
            f"Unexpected data format in payload: {location}, reason: {validation_result}")
        return
